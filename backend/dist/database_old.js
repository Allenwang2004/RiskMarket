...
class DatabaseManager {
    constructor(dbPath = 'risk_market.db') {
        ...
        const fullPath = path_1.default.resol; // 清空所有資料 (僅用於測試)
        clearAllData();
        void {
        ...
        // 獲取價格歷史數據
        getPriceHistory(tokenType, 'yes' | 'no', timeRange, string);
        any[];
        {
      ...
      SELECT 
        ...
        timestamp,
        strftime('%Y-%m-%d %H:00:00', datetime(timestamp/1000, 'unixepoch')) as hour_group
      FROM matches m
    ...
    `);
        ...
        // 獲取價格統計數據
        getPriceStats(tokenType, 'yes' | 'no', timeRange, string = '24h');
        any;
        {
            let hours = 24;
            switch (timeRange) {
    ...
    `);
        // 創建索引提升查詢效能
        this.db.exec(`
      CREATE INDEX IF NOT EXISTS idx_orders_token_type ON orders (token_type);
    ...
    insertOrder(order) {
        const stmt = this.db.prepare(`
      INSERT INTO orders (
        id, user_address, token_type, order_type, price, amount, original_amount, timestamp, status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(order.id, order.userAddress, order.tokenType, order.orderType, order.price, order.amount, order.amount, // 記錄原始數量
    ...
    `;
        ...
        sql += ' ORDER BY ';
        if (orderType === 'buy') {
            sql += 'CAST(price AS REAL) DESC, timestamp ASC'; // 買單按價格降序
        }
        else if (orderType === 'sell') {
            sql += 'CAST(price AS REAL) ASC, timestamp ASC'; // 賣單按價格升序
        }
        else {
            sql += 'order_type, CAST(price AS REAL) DESC, timestamp ASC';
        }
